...
def tela_kanban():
    ...
    # --- 1. Carregar todos os dados (igual a tela_projetos) ---
    df = utils.carregar_projetos_db()
    df_sla = utils.carregar_config_db("sla") 
    df_etapas_config = utils.carregar_config_db("etapas_evolucao") 
    ...
    # --- 2. Definir as colunas do Kanban ---
    # (Você pode ajustar os nomes ou adicionar/remover colunas aqui)
    colunas_kanban = ["BACKLOG", "NÃO INICIADA", "EM ANDAMENTO", "PENDÊNCIA"] 
    
    # (Opcional: Adicionar colunas de finalização)
    # colunas_kanban = ["BACKLOG", "NÃO INICIADA", "EM ANDAMENTO", "PENDÊNCIA", "FINALIZADO"] 

    cols_streamlit = st.columns(len(colunas_kanban))

    # --- 3. Loop por cada coluna definida ---
    for i, col_nome in enumerate(colunas_kanban):
        with cols_streamlit[i]:
            
            # --- 4. Filtrar o DataFrame para esta coluna ---
            df_col = pd.DataFrame() # Inicializa vazio
            
            if col_nome == "BACKLOG":
                # Projetos sem agendamento E que não estejam finalizados/cancelados
                df_col = df[
                    (df['Agendamento'].isna()) & 
                    (~df['Status'].str.lower().isin(['finalizado', 'cancelado', 'finalizada']))
                ]
            elif col_nome == "NÃO INICIADA":
                # Projetos com agendamento E status "NÃO INICIADA"
                df_col = df[
                    (df['Status'].str.lower().str.contains('não iniciad')) & 
                    (df['Agendamento'].notna())
                ]
            elif col_nome == "EM ANDAMENTO":
                df_col = df[df['Status'].str.lower().isin(['em andamento', 'pausado'])] # Inclui pausados aqui
            elif col_nome == "PENDÊNCIA":
                df_col = df[df['Status'].str.lower().str.contains('pendencia')]
            elif col_nome == "FINALIZADO":
                df_col = df[df['Status'].str.lower().str.contains('finalizad|cancelad')]
            
            # --- 5. Desenhar o Título da Coluna ---
            count = len(df_col)
            st.markdown(f"<div class='kanban-column-header'>{col_nome.upper()} ({count})</div>", unsafe_allow_html=True)

            # --- 6. Loop e desenha cada card (versão simplificada) ---
            for _, row in df_col.iterrows():
                project_id = row['ID']
                
                # Coleta dados (igual tela_projetos)
                status_raw = row.get('Status', 'N/A')
                projeto_nome_text = html.escape(str(row.get("Projeto", "N/A"))) 
                agencia_text = html.escape(str(row.get("Agência", "N/A")))
                analista_text = html.escape(str(row.get('Analista', 'N/A')))
                
                # Lógica de SLA e Lembrete (igual tela_projetos)
                sla_text, sla_color_real = utils.calcular_sla(row, df_sla) 
                texto_lembrete_html = ""
                icone_lembrete = ""
                agendamento_date_obj = row.get('Agendamento').date() if pd.notna(row.get('Agendamento')) else None
                if not ('finalizad' in status_raw.lower() or 'cancelad' in status_raw.lower()):
                    if agendamento_date_obj == hoje: 
                         icone_lembrete = "❗"; cor_lembrete = "red"
                ...
                # --- Expander e Formulário de Edição (Copiado de tela_projetos) ---
                # Isso garante que a funcionalidade de editar/salvar/excluir funcione
                with st.expander(f"Ver/Editar Detalhes - ID: {project_id}"):
                    # (Cole aqui o código 'with st.form(...)' COMPLETO de tela_projetos)
                    # (É o bloco exato da linha 600 até 760 da sua tela_projetos)
                    with st.form(f"form_edicao_card_{project_id}"):
                        st.markdown("#### Evolução da Demanda")
                        etapas_do_projeto = df_etapas_config[df_etapas_config["Nome do Projeto"] == row.get("Projeto", "")] if "Nome do Projeto" in df_etapas_config.columns else pd.DataFrame()
                        etapas_concluidas_str = row.get("Etapas Concluidas", ""); etapas_concluidas_lista = []
