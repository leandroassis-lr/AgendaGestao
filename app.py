...
def tela_kanban():
    ...
    for i, col_nome in enumerate(colunas_kanban):
        with cols_streamlit[i]:
            ...
            df_col = pd.DataFrame() 
            if col_nome == "BACKLOG":
                df_col = df[(df['Agendamento'].isna()) & (~df['Status'].str.lower().isin(['finalizado', 'cancelado', 'finalizada']))]
            elif col_nome == "NÃO INICIADA":
                df_col = df[(df['Status'].str.lower().str.contains('não iniciad')) & (df['Agendamento'].notna())]
            elif col_nome == "EM ANDAMENTO":
                df_col = df[df['Status'].str.lower().isin(['em andamento', 'pausado'])] 
            ...
            for _, row in df_col.iterrows():
                ...
                # Lógica de SLA e Lembrete
                sla_text, sla_color_real = utils.calcular_sla(row, df_sla) 
                texto_lembrete_html = ""; icone_lembrete = ""
                agendamento_date_obj = row.get('Agendamento').date() if pd.notna(row.get('Agendamento')) else None 
                ...
                # 1. Botão para acionar o pop-up (type="secondary" usa o CSS que adicionamos)
                if st.button(f"Ver/Editar Detalhes 📝", key=f"btn_kanban_edit_{project_id}", type="secondary"):
                    
                    # 2. O formulário agora está DENTRO do st.dialog
                    with st.dialog(f"Resumo da Atividade: {projeto_nome_text.upper()} (ID: {project_id})", expanded=True):
                        ...
                        with st.form(f"form_edicao_card_kanban_{project_id}"):
                            ...
                            if btn_excluir_card:
                                if utils.excluir_projeto_db(project_id): 
                                    ...
                                    st.rerun() # O rerun vai fechar o dialog
                            if btn_salvar_card:
                                status_final = novo_status_selecionado 
