...
def tela_dados_agencia():
    ...
    for ag in agencias_pag:
        ...
        with st.container():
            ...
            with c1: st.markdown(f"<span style='font-weight:bold; color:{'red' if '🟥' in tag else 'orange' if '🟧' in tag else 'blue'}'>{tag}</span>", unsafe_allow_html=True)
            with c2: st.markdown(f"**{ag}**")
    ...
    if total_pags > 1:
        ...
        with c2: 
            if st.button("⬅️ Anterior", key="bot_prev", disabled=(st.session_state.pag_agencia_atual==0)):
        ...
        with c4:
            if st.button("Próximo ➡️", key="bot_next", disabled=(st.session_state.pag_agencia_atual >= total_pags-1)):
                st.session_state.pag_agencia_atual += 1; st.rerun()
