...
def tela_projetos():
    ...
    # Carrega opções para edição (sem alterações)
    ...
    for _, row in df_paginado.iterrows():
        ...
        projeto_nome_text = html.escape(str(row.get("Projeto", "N/A"))) # Renomeado para clareza
        ...
        # --- >>> NOVA LÓGICA: Encontrar Próxima Etapa <<< ---
        proxima_etapa_texto = "Nenhuma etapa configurada" # Padrão
        
        # 1. Pega as etapas configuradas para ESTE projeto
        ...
        if not etapas_configuradas_df.empty and "Etapa" in etapas_configuradas_df.columns:
            # 2. Pega a lista ordenada de TODAS as etapas possíveis
            todas_etapas_lista = etapas_configuradas_df["Etapa"].astype(str).str.strip().tolist()
            
            # 3. Pega as etapas concluídas DESTE projeto
            ...
            # 4. Encontra a primeira etapa da lista total que NÃO está na lista de concluídas
            proxima_etapa = next((etapa for etapa in todas_etapas_lista if etapa not in etapas_concluidas_lista), None)

            if proxima_etapa:
                proxima_etapa_texto = f"Próxima Etapa: {proxima_etapa}"
            elif len(todas_etapas_lista) > 0: # Se a lista não estava vazia, mas não achou próxima
                 proxima_etapa_texto = "✔️ Todas as etapas concluídas"
            # Se todas_etapas_lista estava vazia, mantém o padrão "Nenhuma etapa configurada"
        # --- >>> FIM DA LÓGICA PRÓXIMA ETAPA <<< ---

        # --- Cabeçalho do Card (Atualizado para incluir próxima etapa) ---
        st.markdown("<div class='project-card'>", unsafe_allow_html=True)
        # Ajuste nas colunas para dar espaço para a nova informação
        col_info_card, col_analista_card, col_agencia_card, col_status_card = st.columns([2.5, 2, 1.5, 1.5]) 
        with col_info_card:
            ...
            # Exibe a próxima etapa aqui
            st.markdown(f"<small style='color: #555;'>{proxima_etapa_texto}</small>", unsafe_allow_html=True) 
        ...
        with st.expander(f"Ver/Editar Detalhes - ID: {project_id}"):
            with st.form(f"form_edicao_card_{project_id}"):
                ...
                # --- Seção de Evolução (Código interno sem alterações) ---
                ...
                novas_etapas_marcadas = [] # Guarda as etapas MARCADAS NESTE FORMULÁRIO
                if not etapas_do_projeto.empty and "Etapa" in etapas_do_projeto.columns:
                    # Guarda a lista completa de etapas possíveis para validação posterior
                    ...
                    todas_etapas_possiveis = [] # Define como lista vazia se não houver etapas
                    total_etapas = 0

                # --- Seções de Informações, Prazos, Detalhes (Código interno sem alterações) ---
                st.markdown("#### Informações e Prazos")
                c1,c2,c3,c4 = st.columns(4)
                # (código dos selectboxes/date_inputs de c1 a c4)
                with c1:
                ...
                # (código dos text_inputs/selectboxes de c5 a c9)
                ...
                # --- Lógica de Ação dos Botões (COM VALIDAÇÃO DE FINALIZAÇÃO) ---
                if btn_excluir_card:
                ...
                if btn_salvar_card:
                    status_final = novo_status_selecionado # Status selecionado no formulário
                    
                    # --- >>> NOVA VALIDAÇÃO AO FINALIZAR <<< ---
                    if 'finalizad' in status_final.lower():
                        # Verifica se TODAS as etapas possíveis foram marcadas AGORA no formulário
                        if total_etapas > 0 and len(novas_etapas_marcadas) < total_etapas:
                            ...
                            st.stop() # Interrompe o salvamento
                        # Verifica se a data de finalização foi preenchida
                        if not _to_date_safe(nova_data_finalizacao): # Usa a data do formulário
                            st.error("ERRO: Se o status é 'Finalizada', a Data de Finalização é obrigatória.", icon="🚨")
                            st.stop() # Interrompe o salvamento
                    # --- >>> FIM DA VALIDAÇÃO <<< ---
                        
                    # Lógica para mudar status para EM ANDAMENTO (sem alterações)
                    ...
                    nova_data_finalizacao_date = _to_date_safe(nova_data_finalizacao) # Usa a data do form
                    novo_agendamento_date = _to_date_safe(novo_agendamento)

                    # Atualiza Log de Agendamento (sem alterações)
                    ...
                    # Monta o dicionário de atualizações (sem alterações)
                    updates = {
                        ...
                        # Salva as etapas MARCADAS NO FORMULÁRIO
                    ...
                    # Chama a função de atualização (sem alterações)
